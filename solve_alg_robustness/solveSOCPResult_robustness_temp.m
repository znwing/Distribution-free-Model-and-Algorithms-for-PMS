    ...
    for sig_ratio = 1:5
        ...
        for m=MachineArray
            for n=[6*m]
                ...
                [time_SOCP(index),x_SOCP,y_SOCP,z_SOCP,obj_SOCP(index),obj_bound_SOCP(index),modeltime_SOCP(index),solvetime_SOCP(index),gap_SOCP(index)]=solveByMixedIntegerSOCPnew_new(m,n,halfsigma,pi,optionsgurobi,constraint,x_con,y_con,z_con,objective, sigma);
                ...
                filename=strcat('./solve_alg_robustness/rubustness_results/GA_result_SOCP_temp/GA_m',num2str(m),'n',num2str(n),'sig_ratio',num2str(sig_ratio),'group',num2str(k),'.mat');
